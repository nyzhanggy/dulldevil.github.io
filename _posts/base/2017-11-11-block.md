...
因为main函数中的局部变量i和函数`__main_block_func_0`不在同一个作用域中，调用过程中只是进行了值传递。**虽然我们可以通过指针来实现局部变量的修改**，但是一般block是作为参数传递以供后续回调执行的，block被执行时，定义时所在的函数栈已经被展开，局部变量已经不在栈中了，再用指针访问就会出现异常。
