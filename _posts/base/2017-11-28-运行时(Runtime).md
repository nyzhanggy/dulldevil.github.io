...
## 运行时
在 Objective-C 中最常被提到的就是运行时（runtime）。抛开语言，单纯看运行时这个概念。运行时，指的就是代码在内存里运行起来的时候，不管什么语言编写的程序都会有运行时的概念。

说到运行时与其相关的还有一个名词叫：编译时，编译时就是编程程序的过程。编写程序用的都是高级语言，像C、C++、JAVA、JavaScript、Objecti-C，这些都是高级语言。但是高级语言在计算机中是不能直接运行的，需要转化成计算机能识别的机器语言，这一步骤可以成为为翻译。  
“编译”就是翻译的一种方式，将高级语言编译成计算机能识别的机器语言。翻译的方式除了编译之外，还有一种叫做“解释”。“解释”与“编译”的最大区别就是翻译的时机不同，“编译”是在程序运行之前，将源码直接翻译成机器语言，然后在交给计算机去执行；“解释”是在程序运行的时候，将源码先翻译成中间代码，再由解释器对中间代码进行解释运行。

在程序运行期间，有的语言可以改变其结构，例如新的函数、对象、甚至代码可以被引进，已有的函数可以被删除或是其他结构上的变化。这样的语言被称为动态语言或动态编程语言（Dynamic Programming Language），像 Objective-C、JavaScript、PHP、Ruby、Python等。

Objective-C 作为动态语言，这之间起到关键作用的就是 Objective-C 中的 Runtime 机制，Runtime 机制中最核心的是：消息机制（Messaging）。
